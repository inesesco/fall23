...
df = pd.read_excel('Total Cost Counts - Language.xlsx')
...
df_filtered = df[(df['PRIMARYLANGUAGE_COUNT'] >= 32) & (df['PRIMARYLANGUAGE'] != "NULL")]
...
for category in claims_categories:
    df_filtered[f'{category} per person'] = (df_filtered[category] / df_filtered['PRIMARYLANGUAGE_COUNT'])
